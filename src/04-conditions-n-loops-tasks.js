...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect1Right <= rect2.left || rect1.left >= rect2Right
    || rect1Bottom <= rect2.top || rect1.top >= rect2Bottom) {
    return false;
  }
  return true;
}
...
function findFirstSingleChar(str) {
  const a = str.split('');
  return a.filter((el) => a.indexOf(el) === a.lastIndexOf(el)).join('')[0];
}
...
function isCreditCardNumber(ccn) {
  const a = ccn.toString().split('');
  let b;
  if (a.length % 2 === 0) {
    b = a.map((el, ind) => (ind % 2 === 0 ? el * 2 : el));
  } else {
    b = a.map((el, ind) => (ind % 2 !== 0 ? el * 2 : el));
  }
  const c = b.map((el) => (el > 9 ? el - 9 : el));
  const k = c.map((el) => (typeof el === 'string' ? Number(el) : el));
  const d = k.reduce((o, u) => o + u, 0);
  return d % 10 === 0;
}
...
function getDigitalRoot(num) {
  const aa = num.toString().split('').reduce((a, b) => +a + +b);
  if (aa > 9) {
    return aa.toString().split('').reduce((a, b) => +a + +b);
  } return aa;
}
